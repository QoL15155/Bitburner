...
export async function main(ns) {
  ...
  if (args.help || args.h || !targetServer || !maxMoney || !minSecurity) {
    ns.tprint(
      `USAGE: run ${ns.getScriptName()} SERVER_NAME SERVER_MAX_MONEY SERVER_MIN_SECURITY_LEVEL`,
    );
    ns.tprint("");
    ...
    ns.tprint(
      "\tSERVER_MAX_MONEY : the maximum amount of money the server can have",
    );
    ns.tprint(
      "\tSERVER_MIN_SECURITY_LEVEL : the minimum security level of the server",
    );
    ns.tprint("");
    ns.tprint("Example:");
