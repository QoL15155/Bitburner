...
export async function main(ns) {
    ...
    function test() {
        if (uniquePathsInGrid([5, 14]) !== 2380) {
            ns.alert(`Test failed for uniquePathsInGrid. Expected [5,14]==2380, but got ${uniquePathsInGrid([5, 14])}`);
        }
        if (uniquePathsInGrid([2, 3]) !== 3) {
            ns.alert(`Test failed for uniquePathsInGrid. Expected [2,3]==3, but got ${uniquePathsInGrid([2, 3])}`);
        }
    }
}