...
export function uniquePathsInGrid1(gridSize) {
    ...
    for (let i = 0; i < rows; i++) {
        result += uniquePathsInGrid1([i + 1, cols - 1]);
    }
    return result
}
...
export async function main(ns) {
    ...
    function test() {
        if (uniquePathsInGrid1([2, 3]) !== 3) {
            ns.alert(`Test failed for uniquePathsInGrid. Expected [2,3]==3, but got ${uniquePathsInGrid1([2, 3])}`);
        }
        if (uniquePathsInGrid1([5, 14]) !== 2380) {
            ns.alert(`Test failed for uniquePathsInGrid. Expected [5,14]==2380, but got ${uniquePathsInGrid1([5, 14])}`);
        }
    }
}