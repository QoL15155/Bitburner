...
export async function main(ns) {
    ...
    if (args.help || args.h) {
        ...
        ns.tprint("=====================");
        ns.tprint("");
        ns.tprint(`Renames all gang members to have the prefix '${memberNamePrefix}'.`);
        ns.tprint("Assumes that you have already created a gang.");
        return;
    }

    const gangMembers = ns.gang.getMemberNames();
    ns.printf(`Current gang members (${gangMembers.length}) : ${gangMembers.join(", ")}`);
    for (let idx=0; idx<gangMembers.length; idx++) {
        ...
        if (!result) {
            printError(ns, `Failed to rename member ${memberName} to ${newMemberName}`);
        }
    }
}
