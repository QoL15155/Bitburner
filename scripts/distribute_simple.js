...
export async function main(ns) {
  ...
  if (args.help || args.h) {
    ...
    ns.tprint(
      "\t--kill_script, -k : kill previous runs of the script on the server before starting new one",
    );
    ns.tprint(
      "\t--kill_all : kill all scripts on the server before starting new one",
    );
    ns.tprint(
  ...
  // Hacked hosts: 44
  print(
    `[${fname}] Distributed ${scriptName} to ${distributedHosts}/${serverList.length} hosts targeting '${targetServerName}'.`,
  );
  ...
  function calculateScriptThreads(serverName) {
    ...
    if (ramDiff < progData.scriptMemory) {
      ns.printf(
        `[${fname}] Memory is full. Max RAM: ${ramMax}, Used RAM: ${ramUsed}`,
      );
      return 0;
    }
    const threads = Math.floor(ramDiff / progData.scriptMemory);
    ns.printf(
      `[${fname}] Max RAM: ${ramMax}, Used RAM: ${ramUsed}, Threads: ${threads}`,
    );
    return threads;
  }
  ...
  function distributeScript(serverName) {
    ...
    const ppid = ns.exec(
      ...
      threads,
      targetServer.name,
      targetServer.maxMoney,
      targetServer.minSecurity,
    );
    if (ppid) {
