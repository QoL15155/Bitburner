...
export async function collectServerInfo(ns, targetName, showServerObject, showPlayerInformation) {
    ...
    if (maxMoney > 0) {
        ...
        const growThreads = ns.growthAnalyze(targetName, moneyMultiplier, serverHome.cpuCores);
        growthObject["growThreads"] = growThreads;
        if (ns.fileExists("Formulas.exe", "home")) {
            const player = ns.getPlayer();
            const growThreadsFormula = ns.formulas.hacking.growThreads(targetObject, player, maxMoney, serverHome.cpuCores);
            growthObject["growThreadFormula"] = growThreadsFormula;
            const growSecurityIncreaseFormula = ns.growthAnalyzeSecurity(growThreadsFormula, targetName, serverHome.cpuCores);
            growthObject["growSecurityIncreaseFormula"] = growSecurityIncreaseFormula;

        }
        const growSecurityIncrease = ns.growthAnalyzeSecurity(growThreads, targetName, serverHome.cpuCores);
    ...
    const security = {
        base: ns.getServerBaseSecurityLevel(targetName),
        minimum: ns.getServerMinSecurityLevel(targetName),
        current: ns.getServerSecurityLevel(targetName),
    }
    ...
    if (showPlayerInformation) {
        const playerObject = {
            homeServerCores: targetObject.cpuCores,
    ...
    if (formulaGrow) {
        ...
        if ((Math.ceil(growThreads) != formulaGrow) && (Math.floor(growThreads) != formulaGrow)) {
            printError(ns, `Grow threads mismatch for ${targetName}. Expected: ${growThreads}, Actual: ${formulaGrow}`);
        }
    }
    return;
