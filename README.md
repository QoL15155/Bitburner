...
Work is distributed across available attacking servers, so overall attack capacity depends on the total RAM available across those servers.

> [!TIP]
> Run `distribute_batch.js` to start running the **batch attack** on target servers.
> Invokes the `controller_batch.js` script. This script uses the other scripts listed below.
...
> [!WARNING]
> This is still a WIP! Currently only used for analyzing the stock market.
...
| `upload_file.js` | Uploads a file from your real computer into Bitburner. ('js' or 'zip' file to get extracted) |
...
6. Create a zip of the *contents* of `scripts/` directory (e.g. with `zip -r scripts.zip scripts/`) and upload it in one shot.
